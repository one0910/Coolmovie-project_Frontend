...
import { transDateString, transLevelData, transMovieTitleName, transTheaterSize } from '../helper/transform.language';
...
export const ScreenCheck: React.FC<ScreenCheckProps> = ({ order, titleMsg, children }) => {
  ...
  const movie_level = (order) ? order.movielevel : state.orderList.movie_level
    ...
    <Aside className='px-3 py-3 mt-5 mt-lg-4' >
      ...
      <div className="screenCheck">
        ...
        <div className='d-flex justify-content-between mb-2'>
          ...
          <span>{`${transMovieTitleName(language, movie_name)}(${movie_level})`}</span>
        </div>
        <div className='d-flex justify-content-between mb-2'>
